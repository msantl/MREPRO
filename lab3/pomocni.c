...
int Socket(int domain, int type, int protocol) {
  ...
  if (fd < 0) {
    Errx(daemon_proc ? LOG_ALERT : 1, "socket error");
  }
...
void Bind(int sockfd, struct sockaddr* addr, socklen_t addr_len) {
  if (bind(sockfd, addr, addr_len) < 0) {
    ...
    Errx(daemon_proc ? LOG_ALERT : 1, "bind error");
  }
}
...
void Connect(int sockfd, struct sockaddr* addr, socklen_t addr_len) {
  if (connect(sockfd, addr, addr_len) < 0) {
    ...
    Errx(daemon_proc ? LOG_ALERT : 1, "connect error");
  }
}
...
int Accept(int sockfd, struct sockaddr* addr, socklen_t *addr_len) {
  ...
  if (fd < 0) {
    ...
    Errx(daemon_proc ? LOG_ALERT : 1, "accept error");
  }
...
void Listen(int sockfd, int backlog) {
  if (listen(sockfd, backlog) < 0) {
    ...
    Errx(daemon_proc ? LOG_ALERT : 1, "listen error");
  }
}
  ...
  while (nleft > 0) {
    if ( (nwritten = write(sockfd, buff, nleft)) <= 0) {
      if (nwritten < 0 && errno == EINTR) {
        ...
        Errx(daemon_proc ? LOG_ALERT : 1, "write error");
      }
    }
    nleft -= nwritten;
    buff += nwritten;
  ...
  while (nleft > 0) {
    if ( (nread = read(sockfd, buff, nleft)) < 0) {
      if (errno == EINTR) {
        ...
        Errx(daemon_proc ? LOG_ALERT : 1, "read error");
      }
    } else if (nread == 0) {
      break; /* EOF */
    }
...
int Send(int sockfd, char *buff, int len, int flags) {
  ...
  if (status < 0) {
    ...
    Errx(daemon_proc ? LOG_ALERT : 1, "send error");
  }
  return status;
}
...
int Recv(int sockfd, char *buff, int len, int flags) {
  ...
  if (status < 0) {
    ...
    Errx(daemon_proc ? LOG_ALERT : 1, "recv error");
  }
  return status;
}
